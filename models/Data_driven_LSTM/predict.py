...
def plot_and_save_prediction(A_real, A_LSTM, sample_id):
    plt.figure(figsize=(10, 4))
    plt.plot(np.arange(len(A_real)), A_real, color='b', markersize=1, label='Real-world')
    plt.plot(np.arange(len(A_real)), A_LSTM, color='r', markersize=1, label='LSTM')
    plt.xlabel('Index')
...
with tf.device('/gpu:0'):
    ...
    test_x = test_x.reshape(test_x.shape[0], 50, 4)
    model = load_model(f"./model/{DataName}.h5")
    test_y_predict = model.predict(test_x)
    test_y_predict = test_y_predict.squeeze(axis=-1)
    ...
    for i in range(1, forward):
        V_LSTM[:, i] = V[:, i - 1] + A_LSTM[:, i - 1] * 0.1

    Y_LSTM = np.zeros_like(Y)
    Y_LSTM[:, 0:2] = Y[:, 0:2]
    for i in range(2, forward):
        Y_LSTM[:, i] = Y[:, i - 1] + V_LSTM[:, i - 1] * 0.1 + A_LSTM[:, i - 1] * 0.005


    # 保存结果
    pd.DataFrame(test_chain_ids).to_csv(f'./results_{DataName}/test_chain_ids.csv', index=False)
    pd.DataFrame(A_LSTM).to_csv(f'./results_{DataName}/A_LSTM.csv', index=False)
    ...
    #     plot_and_save_prediction(A_real[i], A_LSTM[i], i)